...
repo = 'jubatus'
state = 'closed'
milestone = sys.argv[1] # must be a milestone ID (e.g., "21" for 0.5.3)

def fetch(page):
    # fetch from API (no authentication required for public repos)
    url = 'https://api.github.com/repos/%s/%s/issues?milestone=%s&state=%s&page=%d' % (owner, repo, milestone, state, page)
